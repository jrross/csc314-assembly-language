...
int main()
{
  ...
  bigint two64 = bigint_shift_left(one, 64);
  bigint onesChunks = bigint_sub(two64, one);
  bigint shiftedChunks = bigint_shift_left(onesChunks, 64);
  bigint untrimmed = bigint_add(shiftedChunks, onesChunks);
  s = bigint_to_str(untrimmed);
  printf("2^64-1: %s\n", s);
  free(s);
  bigint_free(one);
  bigint_free(two64);
  bigint_free(onesChunks);
  bigint_free(shiftedChunks);
  bigint_free(untrimmed);
